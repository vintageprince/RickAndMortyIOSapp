...
final class RMSearchResultsView: UIView {
    ...
    private let collectionView: UICollectionView = {
        ...
        collectionView.register(RMCharacterCollectionViewCell.self,
                                forCellWithReuseIdentifier: RMCharacterCollectionViewCell.cellIdentifire)
...
extension RMSearchResultsView: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: RMLocationTableViewCell.cellIdentifire,
...
extension RMSearchResultsView: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let characterVN = currentViewModel as? RMCharacterCollectionViewCellViewModel {
            guard let cell = collectionView.dequeueReusableCell(
                withReuseIdentifier: RMCharacterCollectionViewCell.cellIdentifire,
                for: indexPath
            ) as? RMCharacterCollectionViewCell else {
                fatalError()
            }
            
            cell.configure(with: characterVN)
            return cell
        }
        ...
        if let episodeVN = currentViewModel as? RMCharacterEpisodeCollectionViewCellViewModel {
            cell.configure(with: episodeVN)
        }
        return cell
    }
    ...
    private func handleLocationPagination(scrollView: UIScrollView) {
        ...
        Timer.scheduledTimer(withTimeInterval: 0.2, repeats: false) { [weak self] t in
            ...
            if offset >= (totalContentHeight - totalScrollViewFixedHeight - 120) {
                ...
                viewModel.fetchAdditionalLocatons { [weak self] newResults in
                    // Refresh table
                    self?.tableView.tableFooterView = nil
